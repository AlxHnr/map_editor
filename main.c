...
static void load_to_map(const unsigned short step)
{
  ...
  while(current_layer < max_layer)
  {
    ...
    while(x < max_tile_x)
    {
      ...
      while(y < max_tile_y)
      {
        draw_tile(current_layer, x, y, undo_redo[step][current_layer][x][y][0] - ' ', undo_redo[step][current_layer][x][y][1] - ' ');
        y++;
      }
      x++;
    }
...
#endif
{
  ...
  while(program_run)
  {
    ...
    if(show_layer[0])
      draw_layer(0, &camera, screen);
    if(show_layer[1])
      draw_layer(1, &camera, screen);
    if(show_layer[2])
      draw_layer(2, &camera, screen);
    if(show_layer[3])
      draw_layer(3, &camera, screen);
    if(show_layer[4])
      draw_layer(4, &camera, screen);
    if(show_layer[5])
      draw_layer(5, &camera, screen);
    ...
    if(!(show_interface && mouse_x < interface_size))
    {
      ...
      if(draw_state == 1)
      {
        if(current_layer < max_layer - 1)
        {
          while(cache_tile_dest.y < current_tile_dest.y + draw_size)
          {
            ...
            {
              draw_tile(current_layer, cache_tile_dest.x, cache_tile_dest.y, current_tile_src.x/tile_w + 1, current_tile_src.y/tile_h + 1);
              cache_tile_dest.x++;
            }
            cache_tile_dest.y++;
          }
        ...
        else
        {
          while(cache_tile_dest.y < current_tile_dest.y + draw_size)
          {
            ...
            while(cache_tile_dest.x < current_tile_dest.x + draw_size)
            {
              draw_tile(current_layer, cache_tile_dest.x, cache_tile_dest.y, collision_x, collision_y);
              cache_tile_dest.x++;
            }
            cache_tile_dest.y++;
          }
      ...
      else if(draw_state == 2)
      {
        while(cache_tile_dest.y < current_tile_dest.y + draw_size)
        {
          ...
          while(cache_tile_dest.x < current_tile_dest.x + draw_size)
          {
            draw_tile(current_layer, cache_tile_dest.x, cache_tile_dest.y, 0, 0);
            cache_tile_dest.x++;
          }
          cache_tile_dest.y++;
        }
